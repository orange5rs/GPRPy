...
class GPRPyApp:
    ...
    def __init__(self, master):
        ...
        # Show splash screen
        fig = Figure(figsize=(15*self.widfac, 9*self.highfac))
        ax = fig.add_subplot(111)
        dir_path = os.path.dirname(os.path.realpath(__file__))
        splash.showSplash(ax, dir_path, self.widfac, self.highfac, fontfac)

        # Set font size for screen res
        mpl.rcParams.update({'font.size': mpl.rcParams['font.size']*self.widfac})
        ax.tick_params(direction='out',length=6*self.widfac,width=self.highfac)
        ...
        undoButton = tk.Button(
            ...
            command=lambda : [self.resetYrng(proj),
                              self.undo(proj),
                              self.plotProfileData(proj, fig=fig, ax=ax, canvas=canvas)])
        ...
        plotButton = tk.Button(
            text="refresh plot",
            command=lambda : self.plotProfileData(proj, fig=fig, ax=ax, canvas=canvas))
        ...
        DewowButton = tk.Button(
            ...
            command=lambda : [self.dewow(proj),
                              self.plotProfileData(proj, fig=fig, ax=ax, canvas=canvas)])
        DewowButton.config(height=1, width=halfwid)
        DewowButton.grid(row=7, column=rightcol, sticky='nsew')
        self.balloon.bind(DewowButton,
                          "Trace-wise low-cut filter. Removes\n" 
                          "from each trace a running mean of\n"
                          "chosen window width.")

        DewowButton2 = tk.Button(
            text="dewow2", fg="black",
            command=lambda : [self.dewow2(proj),
                              self.plotProfileData(proj, fig=fig, ax=ax, canvas=canvas)])
        DewowButton2.config(height=1, width=halfwid)
        DewowButton2.grid(row=7, column=rightcol+1, sticky='nsew')
        self.balloon.bind(DewowButton2,
                          "Trace-wise low-cut filter. Removes\n"
                          "from each trace a running mean of\n"
                          "chosen window width.")
        ...
        hypButton = tk.Button(
            ...
            command=lambda : [self.showHyp(proj, ax), canvas.draw()])
        ...
        startPickButton = tk.Button(
            ...
            command=lambda : self.startPicking(proj, fig=fig, ax=ax, canvas=canvas))
    ...
    def startPicking(self, proj, fig, ax, canvas):
        self.picking = True
    ...
    def plotProfileData(self, proj, fig, ax, canvas):
        ...
        if proj.velocity is None:
            ax.imshow(proj.data, cmap=self.color.get(), extent=[min(proj.profilePos)-dx/2.0,
            ...
            ax.set_ylim(self.yrng)
            ax.set_xlim(self.xrng)
            ax.set_ylabel("Two-way travel time [ns]", fontsize=mpl.rcParams['font.size'])
            ax.invert_yaxis()
            ...
            ax.imshow(proj.data,cmap=self.color.get(),extent=[min(proj.profilePos)-dx/2.0,
                     ...
                     interpolation='none',
                     vmin=-stdcont/self.contrast.get(), vmax=stdcont/self.contrast.get())
            ax.set_ylabel("Depth [m]", fontsize=mpl.rcParams['font.size'])
            ax.set_ylim(self.yrng)
            ax.set_xlim(self.xrng)
            ax.invert_yaxis()
            ...
            ax.imshow(proj.data,cmap=self.color.get(),extent=[min(proj.profilePos)-dx/2.0,
                     ...
                     interpolation='none',
                     vmin=-stdcont/self.contrast.get(), vmax=stdcont/self.contrast.get())
            ax.set_ylabel("Elevation [m]", fontsize=mpl.rcParams['font.size'])
            ax.set_ylim(self.yrng)
            ax.set_xlim(self.xrng)

        ax.get_xaxis().set_visible(True)
        ax.get_yaxis().set_visible(True)
        ax.set_xlabel("Profile position [m]", fontsize=mpl.rcParams['font.size'])
        ax.xaxis.tick_top()
        ax.xaxis.set_label_position('top')
        if self.asp is not None:
            ax.set_aspect(self.asp)
        ...
        if self.picking:
           ax.plot(self.picked[:,0], self.picked[:,1], '-x', color='yellow',linewidth=3*self.highfac)
           ax.plot(self.picked[:,0], self.picked[:,1], '-x', color='black', linewidth=2*self.highfac)
